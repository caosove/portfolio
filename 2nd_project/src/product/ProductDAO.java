...
public class ProductDAO extends JdbcDAO {
	...
	public List<ProductDTO> getProductList2(int startRow,int endRow, String menu, String sorting) {
		...
		try {
			...
			if (menu.equals("NEW")){
				sql +="select * from product_shopping where p_status!='STOP' order by " + sorting + " desc) temp) where rn between ? and ?";				
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, startRow);
				pstmt.setInt(2, endRow);
			} else if (menu.equals("ALL")){
				sql +="select * from product_shopping where p_status!='STOP' order by " + sorting + ") temp) where rn between ? and ?";				
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, startRow);
				pstmt.setInt(2, endRow);
			} else {
				sql += "select * from product_shopping where p_type='" + menu + "' and p_status!='STOP' order by " + sorting + " ) temp) where rn between ? and ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, startRow);
				pstmt.setInt(2, endRow);
			}
	...
	//메뉴와 검색조건에 따라 검색된 제품의 갯수 반환
	  /* public int getTotalProduct(String menu, String sorting) {
	      ...
	      try {
	         con = getConnection();
	        if (menu.equals("NEW")) {
	            String sql ="select count(*) from product_shopping where p_status!='STOP'order by " + sorting;
	            pstmt = con.prepareStatement(sql);
	         } else if (menu.equals("ALL")) {
	            String sql ="select count(*) from product_shopping where p_status!='STOP' order by " + sorting;
	            pstmt = con.prepareStatement(sql);
	         } else {
	        	 String sql ="select count(*) from product_shopping where p_Type='"+menu+"' and p_status!='STOP' order by " + sorting;
	         }
