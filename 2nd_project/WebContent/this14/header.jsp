...
<html>
	<head>
		...
		<link href="../product/css/reset.css" rel="stylesheet" type="text/css">
		<link href="../product/css/main.css" rel="stylesheet" type="text/css">	
		...
		<div id="ikea" class="ikea-new-ui-body ikea-new-ui-header">
			<header class="ikea2017-header KO">
				<section class="top-nav">		
					<nav class="wrap rel">
						<div class="welcome-msg">Welcome to a world of inspiration for your home. 안녕하세요. IKEA Korea 입니다. </div>
				...
				<section class="logo-search-box">
					<div class="wrap rel">
						<h1 class="ikea-logo"><a href="<%=request.getContextPath() %>/this14/product/index.jsp"><img src="https://uiux.newideas.kr/images/ikea-logo.svg" alt="IKEA"></a></h1>						
						...
						<div class="cart-container">
							...
							<a href="<%=request.getContextPath() %>/this14/Cart/cart2.jsp">
								 <span class="ico-cart"></span></a>
								<span id="my-cart-quantity" class="cart-count">0</span>
							<%} %>	
						...
						<div class="btn-cart-wrap">
							...
							<%} else if(!loginUser.getuGrade().equals("admin")){ %>
							<a href="<%=request.getContextPath() %>/this14/Cart/Buy_List.jsp">
							<span class="alttxt">카트 보기</span></a>
							<%} %>
						</div>
					</div>				
				</section>
  ...
  href="<%=request.getContextPath() %>/this14/product/index.jsp?workgroup=product&work=product_list&menu=ALL">모든 제품</a>
  								</li>
									<li class="new-products"><a href="<%=request.getContextPath() %>/this14/product/index.jsp?workgroup=product&work=product_list&menu=NEW">New</a></li>
							<li class="all-products">
							    <!-- 자바스크립트로 대메뉴시퀀스을 ID로 가진 하위 DIV를 오픈한다. -->
							    <a href="<%=request.getContextPath() %>/this14/product/index.jsp?workgroup=product&work=product_list&menu=LIVING">소파·의자·테이블</a>
							</li>
							<li class="all-products">
							    <!-- 자바스크립트로 대메뉴시퀀스을 ID로 가진 하위 DIV를 오픈한다. -->
								<a href="<%=request.getContextPath() %>/this14/product/index.jsp?workgroup=product&work=product_list&menu=BED">침대·매트리스</a>
					        </li>
							<li class="all-products">
							    <!-- 자바스크립트로 대메뉴시퀀스을 ID로 가진 하위 DIV를 오픈한다. -->
								<a href="<%=request.getContextPath() %>/this14/product/index.jsp?workgroup=product&work=product_list&menu=KITCHEN">요리·식기·주방가구</a>
							</li>
							<li class="all-products">
					            <!-- 자바스크립트로 대메뉴시퀀스을 ID로 가진 하위 DIV를 오픈한다. -->
					            <a href="<%=request.getContextPath() %>/this14/product/index.jsp?workgroup=product&work=product_list&menu=TEXTILE">텍스타일·러그·커튼</a>
					        </li>
    ...
    border-left: 1px solid rgba(255,255,255,.15);padding-top:0" class="nav-right-banner showroom" href="<%=request.getContextPath()%>/this14/admin/indexAdmin.jsp">관리자</a>
	         ...
	         <div class="login-content"> 
	             ...
	             <form action="<%=request.getContextPath() %>/this14/login/login_action.jsp" method="POST" class="login-form" name="login_form"> 
	               ...
	               <div class="login-footer"><a href="<%=request.getContextPath() %>/this14/login/lookup_info.jsp" class="login-join">아이디/비밀번호 찾기</a>&nbsp;│&nbsp;<a href="<%=request.getContextPath() %>/this14/login/join_step_01.jsp" class="login-join">회원가입</a></div> 
	             </form> 
	         </div> 
	    </div>
