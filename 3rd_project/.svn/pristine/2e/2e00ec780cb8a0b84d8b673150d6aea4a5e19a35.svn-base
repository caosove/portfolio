 ...
 */
	   ...
	   $(".previewBtn").click(function () {
		   $("#error").html("");
			...
			if(type!="7" && fileType!="xlsx"){
	    		$("#msg").html("엑셀파일(.xlsx)을 선택해 주세요.");
				return false;
			} else if(type=="7" && fileType!="csv"){
				$("#msg").html("CSV파일(.csv)을 선택해 주세요.");
				return false;
			} 
	    		...
	    		beforeSend: function(){
	    			...
	    			var cnt=setInterval(function() {
						$.ajax({
							...
							dataType: "text",
							success: function(text){
								$("#msg").html("업로드 진행 중 입니다.<br>시간이 다소 소요될 수 있습니다.<br>..."+text+"% 진행중...");
								if(text=="100"){
									clearInterval(cnt);
									$("#msg").html("업로드가 완료 되었습니다.");
									$("#loadingImg").hide();
								}
							},
							error: function(req){
	   ...
	   $("#logout").click(function(){
			location.href="/kingsmap/admin/logout"
	   });
